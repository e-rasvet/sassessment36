        ...
        if ($list->uid == $USER->id || has_capability('mod/sassessment:teacher', $context)) { // Only for Teachers and owners
            ...
            for ($i = 1; $i <= 10; $i++) {
                if (!empty($list->{'var' . $i}) || !empty($list->{'file' . $i})) {
                    ...
                    if (!empty($list->{'var' . $i}) && $sassessment->transcribe == 1)
                        $o .= '<b>' . get_string("studentresponse", "sassessment") . ":</b> " . $studentPlayer . " " . $list->{'var' . $i};
                    else if (!empty($studentPlayer))
                        $o .= '<b>' . get_string("studentresponse", "sassessment") . ":</b> " . $studentPlayer;
                    ...
                    if ($response)
                        $o .= "<div style='font-size: small;color: #888;'><b>" . get_string("sampleresponse", "sassessment") . ":</b> " . $targetAnswerPlayer . " " . $response->text . '</div>';
                    else
                        $o .= "<div style='font-size: small;color: #888;'><b>" . get_string("sampleresponse", "sassessment") . ":</b> " . "No response" . '</div>';

                    if ($sassessment->{'var' . $i})
                        $o .= "<div style='font-size: small;color: #888;'><b>" . get_string("questionpromt", "sassessment") . ":</b> " . $targetPlayer . " " . $sassessment->{'var' . $i} . '</div>';
                    else
                        $o .= "<div style='font-size: small;color: #888;'><b>" . get_string("questionpromt", "sassessment") . ":</b> " . "No response" . '</div>';
    ...
    class sassessment_comment_form extends moodleform
    {
        function definition()
        {
          ...
          <td style="width:50px;"><img src="img/speak.png" /></td>';
                    ...
                    if ($sassessment->transcribe == 0)
                        $hiddenTranscribeStatus = ' style="display: none"';
                    else
                        $hiddenTranscribeStatus = ' ';

                    $o .= '<td style="">
              <div id="answer_div_' . $i . '" data-url="' . str_replace('"', "'", $response->text) . '" '.$hiddenTranscribeStatus.'></div>
